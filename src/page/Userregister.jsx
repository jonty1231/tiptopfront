...
const registerhandel=async(e)=>{
    ...
    try {
        if(name && email && password){
            const res =await axios.post("http://localhost:8080/user/register",{name,email,password})
            if(res.data.success){
  ...
  return (
    ...
    <form className='shadow-2xl p-4 bg-white sm:w-[100%] sm:h-[100%] sm:pt-[10rem]' onSubmit={registerhandel}>
       ...
       <div className='m-2 text-2xl'>
        ...
        <div className='flex justify-between text-blue-700 sm:text-[1.2rem]'> <Link to='/login'>Allready Have a Account?</Link></div>
    </form> </div>
  )
}
